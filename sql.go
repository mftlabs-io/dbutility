...
// archiveOrder is the order tables are archived/deleted in; keyed into historySpecs.
var archiveOrder = []string{"amf_message", "amf_session", "amf_session_rel", "amf_event"}

var historySpecs = map[string]historySpec{
	"amf_message": {
		historyTable: "amf_message_history",
		insertCols:   "sender,receiver,msg_type,control_no,file_name,file_type,file_path,workflow_id,session_id,parent_id,doc_count,origin,reference_id,status,status_time,can_requeue,can_reprocess,create_time,created_by,message_id,file_size,site_id,node_id,can_req_and_rep,data_type,contents",
		selectCols:   "sender,receiver,msg_type,control_no,file_name,file_type,file_path,workflow_id,session_id,parent_id,doc_count,origin,reference_id,status,status_time::timestamptz,can_requeue,can_reprocess,create_time::timestamptz,created_by,message_id,file_size,site_id,node_id,can_req_and_rep,data_type,contents",
	},
	"amf_session": {
		historyTable: "amf_session_history",
		insertCols:   "session_id,session_start,session_end,workflow_name,instance_id,username,status,create_time,created_by,site_id,node_id",
		selectCols:   "session_id,session_start,session_end,workflow_name,instance_id,username,status,create_time,created_by,site_id,node_id",
	},
	"amf_session_rel": {
		historyTable: "amf_session_rel_history",
		insertCols:   "relation_id,session_id,message_id,rel_type,create_time,created_by",
		selectCols:   "relation_id,session_id,message_id,rel_type,create_time,created_by",
	},
	"amf_event": {
		historyTable: "amf_event_history",
		insertCols:   "event_id,level,message_id,session_id,action_id,text,status,create_time,created_by",
		selectCols:   "event_id,level,message_id,session_id,action_id,text,status,create_time,created_by",
	},
}

// MoveToHistory archives rows into the history table and removes them from the
// main table in ONE statement: the delete and insert run atomically, the main
// table is scanned once (instead of count + insert-select + delete = three
// scans), and rows already present in history from a previous partially
// completed run are skipped via ON CONFLICT DO NOTHING instead of aborting
// the whole batch with a duplicate key error.
// Pass fromdate == "" to move everything up to todate.
func (util *DbUtil) MoveToHistory(context utilities.AppContext, Db *dbconnector.DbConnector, fromdate, todate, tablename string) error {
	spec, ok := historySpecs[tablename]
	if !ok {
		return fmt.Errorf("MoveToHistory: unknown table %q", tablename)
	}
	where := " where create_time <= $1"
	args := []interface{}{todate}
	if fromdate != "" {
		where = " where create_time >= $1 and create_time <= $2"
		args = []interface{}{fromdate, todate}
	}
	query := "with moved as (delete from " + tablename + where + " returning *) " +
		"insert into " + spec.historyTable + " (" + spec.insertCols + ") " +
		"select " + spec.selectCols + " from moved on conflict do nothing"
	resp, err := Db.Exec(query, args...)
	if err != nil {
		return err
	}
	rowcount, _ := resp.RowsAffected()
	context.Logger.Info("Moved %v rows from %v to %v\n", rowcount, tablename, spec.historyTable)
	return nil
}
